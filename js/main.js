import { deleteBook, getAllBooks, postBook } from "./api.js";
...
const item = ({ id, title, author, pages, price }) => {
  const list = document.querySelector("#item-list");
  const item = document.createElement("div");

  item.innerHTML = `
	<div id="${id}"class="card m-5" style="width: 18rem;">
	<img class="card-img-top" src="img/book.svg" alt="Card image cap">
	<div class="card-body">
		<h4 class="card-title">${title}</h4>
		<h6 class="card-subtitle text-muted my-2">${author}</h6>
		<h6 class="card-subtitle text-muted">${pages} pages</h6>
		<h5 class="card-subtitle pt-3">${price} UAH</h6>
		<button  id="delete-button-${id}" type="button" class="btn btn-danger btn-sm float-end delete-btn">Delete</button>
		<button id="edit-button-${id}" type="button" class="btn btn-primary btn-sm float-end mx-1 edit-btn "><i class="bi bi-pencil-fill"></i></button>
	</div>
</div>`;
...
document.querySelector("#create_page-form").addEventListener("submit", (e) => {
  ...
  if (title === "" || author === "" || pages === "" || price === "") {
    ...
    postBook({ title, author, pages, price }).then(refetchAllBooks);
...
document.getElementById(`#edit-button-${id}`).addEventListener("click", () => {
...
document
  .querySelectorAll(`#delete-button-${id}`)
  .addEventListener("click", () => {
    console.log("ghjf");
    const onRemove = (element) => {
      deleteBook(element.id).then(refetchAllBooks);
    };
  });
...
document.querySelector("#search-btn").addEventListener("click", (e) => {
  ...
  const foundBooks = books.filter(
    (book) =>
      book.title.toUpperCase().search(searchInput.value.toUpperCase()) !== -1
  );
